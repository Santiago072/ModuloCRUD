    ...
    <div className="fixed inset-0 bg-black/60 backdrop-blur-xs flex items-center justify-center z-50 p-3 sm:p-4">
      <div className="bg-white rounded-2xl sm:rounded-3xl shadow-2xl w-full max-w-lg max-h-[90vh] overflow-y-auto">
        ...
        <div className="p-4 sm:p-6 space-y-5">
          ...
          <div>
            <div className="flex items-center justify-between mb-2">
              <p className="text-xs text-gray-400 uppercase tracking-wide">Números de Contacto</p>
              {!addingContacto && (
                <button onClick={() => setAddingContacto(true)} className="text-xs text-indigo-600 hover:text-indigo-800 flex items-center gap-1 font-medium">
            ...
            {addingContacto && (
              <div className="flex gap-2 mb-3">
                <input type="tel" value={newContactoVal} onChange={e => setNewContactoVal(e.target.value)}
                  placeholder="Número de celular" className="flex-1 border border-indigo-300 rounded-lg px-3 py-1.5 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500" />
                <button onClick={handleAddContacto} className="px-3 py-1.5 bg-indigo-600 text-white text-xs rounded-lg hover:bg-indigo-700 font-medium">Guardar</button>
                <button onClick={() => { setAddingContacto(false); setNewContactoVal(''); }} className="px-3 py-1.5 text-gray-500 text-xs rounded-lg hover:bg-gray-100">Cancelar</button>
              </div>
            )}
              ...
              {contactos.length === 0 ? (
                ...
                contactos.map((c) => (
                  <div key={c.id} className={`flex items-center justify-between p-3 rounded-xl border text-sm ${
                    c.prioridad === 1 ? 'bg-indigo-50 border-indigo-100 text-indigo-900 font-medium' : 'bg-gray-50 border-gray-100 text-gray-700'
                  }`}>
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <button onClick={() => handleDeleteContacto(c.id)} className="text-gray-300 hover:text-red-500 p-1 transition-colors">
                        <Trash2 size={13} />
                      </button>
                    </div>
                  </div>
        ...
        <div className="p-4 sm:p-6 pt-0 flex flex-col sm:flex-row justify-between items-stretch sm:items-center gap-3">
          {!confirmDelete ? (
            <button onClick={() => setConfirmDelete(true)} className="flex items-center justify-center sm:justify-start gap-1.5 text-sm text-red-500 hover:text-red-700 font-medium py-1">
          ...
          <div className="flex gap-2 justify-end">
            {editMode ? (
              <>
                <button onClick={() => setEditMode(false)} className="flex-1 sm:flex-none px-4 py-2 text-sm text-gray-600 bg-gray-100 hover:bg-gray-200 rounded-lg transition-colors font-medium">Cancelar</button>
                <button onClick={handleSave} disabled={loading} className="flex-1 sm:flex-none flex items-center justify-center gap-1.5 px-4 py-2 text-sm bg-indigo-600 hover:bg-indigo-700 text-white font-medium rounded-lg disabled:opacity-60 transition-colors shadow-sm">
                  <Save size={14} /> Guardar
                </button>
              </>
            ) : (
              <button onClick={() => setEditMode(true)} className="w-full sm:w-auto px-5 py-2 text-sm bg-indigo-50 hover:bg-indigo-100 text-indigo-700 font-medium rounded-lg transition-colors text-center">
                Editar
              </button>
            )}
          </div>
